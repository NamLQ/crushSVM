...
namespace shark {
    ...
    void DataModelContainer::getModelName (std::string &modelName) {
        modelName = m_modelName;
        BOOST_LOG_TRIVIAL (trace) << "Retrieving modelname.";
    }

    
    void DataModelContainer::setModelName (std::string modelName) {
        m_modelName = modelName;
        BOOST_LOG_TRIVIAL (trace) << "Setting modelname.";
    }
    ...
    void DataModelContainer::loadSparseLabelAndData (std::ifstream &stream) {
        ...
        while (stream) {
            ...
            // if we have an comment inbetween, we interprete this as a sign to stop reading.
            // this is needed for swarmsvm-file format
            boost::cmatch matchedStrings;
            boost::regex SwarmSVMExpression ("^#");
            
            if (boost::regex_search (line.c_str(), matchedStrings, SwarmSVMExpression )) {
                // stop reading but rewind, as this stupid comment might be part of the swarmsvm model yes FIXME
                stream.seekg(place);
                break;
            }
	    ...
	    boost::split( SplitVec, line, boost::algorithm::is_any_of(" "), boost::token_compress_on ); 
	  
	    // first one is the label
            typedef std::pair<std::vector<double>, std::vector<std::pair<std::size_t, double> > > LibSVMPoint;
	    
	    std::vector<double> labels;
	    std::vector<std::pair<std::size_t, double> > features;
	    for (int i = 0; i < SplitVec.size(); i++) {
                std::string s = SplitVec[i];
                if (s.find(':') != std::string::npos) {
                    // must be a feature
                    // we have a ':', so we split and add
                    split_vector_type f;
                    boost::split( f, s, boost::algorithm::is_any_of(":"), boost::token_compress_on ); 
                    features.push_back(std::pair<std::size_t, double>(boost::lexical_cast<std::size_t>(s[0]), boost::lexical_cast<double>(s[1])));
                } else {
                    labels.push_back(boost::lexical_cast<double>(s));
                }
	    }
	    LibSVMPoint newPoint (labels, features);
