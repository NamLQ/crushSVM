...
namespace shark {
    ...
    SVMDataModel SVMDataModelFactory::createFromFile (std::string filePath) {
        ...
        for (size_t i = 0; i < 24; i++) {
            ...
            // check for swarmsvm
            boost::regex SwarmSVMExpression ("# swarmsvm");
            
            if (boost::regex_search (line.c_str(), matchedStrings, SwarmSVMExpression)) {
                BOOST_LOG_TRIVIAL (info) << "Found swarmsvm Model";
                model = SVMDataModel (new SwarmSVMDataModel);
                break;
            }
    ...
    SVMDataModel SVMDataModelFactory::createFromString (std::string modelName)
    {
        ...
        SVMDataModel model;
        if  ((modelName == "LibSVM") || (modelName == "LIBSVM")) {
