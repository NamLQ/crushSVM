...
int main (int argc, char** argv) {
    ...
    try {
        ...
        double cost = -42.0;
        uint32_t crossValidationFolds = 1;
        ...
        if (crossValidationFolds > 1) {
            // we have X models at our hand.
            // for now we do two things and report the values:
            // a) create an ensemble by majority voting
            // b) take average of alpha values
            
            // load all our models
            modelDataPath = vm["model_file"].as<std::string>();
            
            std::vector< KernelClassifier<RealVector>  > foldClassifiers(crossValidationFolds);
            std::vector< KernelExpansion<RealVector>  > foldModels(crossValidationFolds);
            for (size_t f = 0; f < crossValidationFolds; f++) {
                std::stringstream tmpS;
                tmpS << modelDataPath << f+1 << "_" << crossValidationFolds << ".model";
                std::string foldModelFilename = tmpS.str();
                BOOST_LOG_TRIVIAL (info) << "Loading model from " << foldModelFilename ;

                SVMDataModel svmModel = SVMDataModelFactory::createFromFile (foldModelFilename);
                svmModel->load (foldModelFilename);
                ...
                BOOST_LOG_TRIVIAL (debug) << "Found  " << nClasses << " alpha-coefficients per SV, and will work with " << nBiasTerms << " bias terms. ";
                ...
                foldModels[f] = KernelExpansion<RealVector> (kernel, supportVectors, true, nClasses);
                ...
                foldModels[f].setParameterVector (parameters);
                
                BOOST_LOG_TRIVIAL (info) << "Model has Offset? " << foldModels[f].hasOffset();
                
                if (foldModels[f].hasOffset()) {
                    BOOST_LOG_TRIVIAL (debug) << "Size of offset " << foldModels[f].offset().size();
                    
                    for (size_t c = 0; c < nBiasTerms; c++) {
                        BOOST_LOG_TRIVIAL (trace) << "Offset is " << foldModels[f].offset() [c];
                    }
                }
                
                // create a classifier with the model
                foldClassifiers[f] = KernelClassifier<RealVector> (foldModels[f]);
            }
            ...
            // HACK: assume here that the  label order is all the same for all models
            ...
            std::vector<Data<unsigned int> > foldPredictions (crossValidationFolds);
            for (size_t f = 0; f < crossValidationFolds; f++) {
                // do prediction each for itself
                ZeroOneLoss<unsigned int> loss;
                foldPredictions[f] = foldClassifiers[f] (testData.inputs());
                double error_rate = loss (testData.labels(), foldPredictions[f]);
                
                BOOST_LOG_TRIVIAL (info) << "Test error rate for fold "  << f << ": " << error_rate;
            }
            
            // HACK: stupid initialization
            Data<unsigned int> ensemblePrediction = foldPredictions[0];
            for (size_t i = 0; i < testData.numberOfElements(); i++) {
                double vote = 0;
                for (size_t f = 0; f < crossValidationFolds; f++) {
                    vote += foldPredictions[f].element(i);
                }
                vote = vote/crossValidationFolds;
                ensemblePrediction.element(i) = round(vote);
            }
            // do prediction each for itself
            ZeroOneLoss<unsigned int> loss;
            double error_rate = loss (testData.labels(), ensemblePrediction);
            
            BOOST_LOG_TRIVIAL (info) << "Ensemble (avg/majority vote) error rate: " << error_rate;

            
            // create alpha averaged model
            std::stringstream tmpA;
            tmpA << modelDataPath << "_" << crossValidationFolds << ".alpha";
            std::ifstream ifs;
            ifs.open (tmpA.str().c_str());
