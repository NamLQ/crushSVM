	...
	double Cp, double Cn, int solver_type)
{
	...
	while (iter < max_iter)
	{
		...
		for (i=0; i<slen; i++) swap(index[i], index[rand() % slen]);
		...
		for (s=0; s<slen; s++)
		{
			...
			// update gain-based preferences
			{
				...
				else
				{
					...
					double newpref = min(PREF_MAX, max(PREF_MIN, pref[i] * exp(change)));
	...
	double seconds = finish.tv_sec - start.tv_sec + 1e-6 * (finish.tv_usec - start.tv_usec);
	info("optimization time: %7.3g seconds\n", seconds);
	info("update steps: %lu\n", steps);
...
void solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp, double Cn)
{
	...
	double seconds = finish.tv_sec - start.tv_sec + 1e-6 * (finish.tv_usec - start.tv_usec);
	info("optimization time: %11.7g seconds\n", seconds);
	info("update steps: %lu\n", steps);
