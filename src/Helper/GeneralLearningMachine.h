...
public:
    ...
    void train() 
    {
        ...
        if (clp.crossValidationFolds > 1) {
            ...
            // create folds 
            RecreationIndices foldIndices;
            CVFolds<ClassificationDataset> folds = createCVSameSizeBalanced(trainingData, clp.crossValidationFolds, ClassificationDataset::DefaultBatchSize, &foldIndices);
            std::vector<double> cvFoldErrors;
            
            std::vector<double> alphaVector (trainingData.numberOfElements());
            for (size_t f = 0; f < folds.size(); f++) {
                ...
                std::cout << "Fold has " << fold.numberOfElements() << " elements.\n";
                ...
                // HACK: alpha average-- 
                // get indices-- this is now a normal vector of unsigned ints
                RealMatrix fAlpha = kernelClassifier.decisionFunction().alpha();
                Data<RealVector> supportVectors;
                supportVectors = kernelClassifier.decisionFunction().basis();
                
                
                // HACK: because shark is soo not cool. 
                for (size_t x = 0; x < supportVectors.numberOfElements(); x++) {
                    RealVector b = supportVectors.element(x);

                    int match = -1;
                    for (size_t r = 0; r < trainingData.numberOfElements(); r++) {
                        RealVector a = trainingData.inputs().element (r);
                        ...
                        if (same == true) {
                            ...
                            alphaVector[ r ] += (currentAlpha);
                            break;
                        }
                    }
                    
                    if (match == -1) {
                        throw (SHARKSVMEXCEPTION ("apples! pies!!"));
                    }
                }
            ...
            // HACK: create true vector from matrix 
                    for (std::size_t j = 0; j < alphaVector.size(); j++) {
                        // every SV was trained exactly folds-1 times
                        alphaVector[j] /= (folds.size()-1);
                    }
                
                    // HACK: again assume no bias here
                    size_t nBiasTerms = 1;
                    ...
                    RealVector parameters (alphaVector.size() + nBiasTerms);
                    
                    // now copy over all alphas into the parameter vector
                    for (size_t c = 0; c <  alphaVector.size(); c++) {
                        parameters[c] = alphaVector[c];
                    }
                    
                    // put all bias terms at the end
                    parameters[parameters.size() - 1] = 0;
                    
                   
                    // now set all structure and save the averaged model as a file
                   
                    
                    // have model its parameters.
                    kernelClassifier.setParameterVector (parameters);

                     std::stringstream tmpS;
                     tmpS << clp.modelDataPath << "_" << folds.size() << ".alpha";
//                     std::ofstream ofs;
//                     ofs.open (tmpS.str().c_str());
//                     
//                     if (!ofs) throw (SHARKSVMEXCEPTION ("File can not be opened for writing!"));
//                     ofs.close();
