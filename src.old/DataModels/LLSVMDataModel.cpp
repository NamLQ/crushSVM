...
namespace shark {
    ...
    void LLSVMDataModel::load (std::ifstream &inputStream) {
        if (!inputStream.good())
            throw SHARKSVMEXCEPTION ("LLSVMDataModel failed to open file for input");

        // read header and data
        loadHeader (inputStream);
        loadSparseLabelAndData (container, inputStream);
    }



    void LLSVMDataModel::save (std::ofstream &outputStream) {
        if (!outputStream)
            throw (SHARKSVMEXCEPTION ("output stream can not be used for writing"));

        // create header and save data
        saveHeader (outputStream);
        RealMatrix preparedAlphas = container -> m_alphas;
        container -> saveSparseLabelAndData (outputStream);
    }
